...
public class TVAdapter extends ArrayAdapter<TV> {
    ...
    private final List<TV> lista;
    private final SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy", Locale.getDefault());

    public TVAdapter(Context context, List<TV> lista) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        tvMarca.setText(tv.getMarca() + " (ID: " + tv.getId() + ")");
        tvDetalii.setText(tv.getDiagonala() + "\" | " + tv.getTipPanel() + " | " + tv.getPret() + " RON");

        if (tv.getDataAdaugarii() != null) {
            tvData.setText("Adaugat: " + sdf.format(tv.getDataAdaugarii()));
        } else {
            tvData.setText("");
        }
